...
namespace Supervertaler.Core
{
    ...
    /// is selected. One wording for every Supervertaler product.
    ///
    /// <para>Written after a live memoQ job, where the model appended an English
    /// note addressed to the translator, in markdown, to a Dutch target, and the
    /// plugin wrote it into the grid. memoQ has no base system prompt of its own -
    /// the library prompt IS the system prompt - so what a reply could contain was
    /// whatever the selected prompt happened to say, and the Default Translation
    /// Prompt said nothing firm. The base prompt Trados wraps around every prompt
    /// forbids commentary in one line, and gives a model no sanctioned place to
    /// put a remark it thinks matters.</para>
    ///
    /// <para>So a host puts this on the end of every system prompt, after the
    /// library prompt and anything else it adds, where a prompt cannot switch it
    /// off by accident. A prompt may add to it - its own examples of good
    /// comments, a different language for them - but the shape of the reply is
    /// fixed here. It is the same text on every request of a job, so it sits
    /// inside the provider's cached prefix.</para>
    ///
