...
namespace Supervertaler.Core
{
    ...
    /// document. On a live memoQ job that was a Dutch target followed by an
    /// English note, in markdown, explaining a choice to the translator - no
    /// marker, no line break, nothing a later step could have told apart from the
    /// translation. <see cref="OutputContract"/> says what a reply may contain;
    /// this is what makes that more than a request.</para>
    ///
    /// <para>Framework only: it judges two strings, the source as the model saw
    /// it and one segment of its reply, both in the host's own tagged form. What
    /// a host does with a refusal - ask again, leave the row, tell the user - is
    /// the host's.</para>
    ///
    ...
    ///
    /// <para>Tags are NOT a reason to refuse. memoQ's staging check came to the
    /// same conclusion for the same reason: a target that legitimately drops a
    ...
    /// </summary>
    public static class ReplyCheck
    {
        /// <summary>
        ...
        // Both bracket forms. [[TC: ...]] is the standard; prompts written for
        ...
        /// null when they match. Names are the identity: memoQ hands a plugin text
        /// markers rather than tag objects, and Trados's placeholders are numbered
        /// names too, so the name is what both can compare.
        /// </summary>
